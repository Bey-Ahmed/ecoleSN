...
for year in range(2012, 2017, 1) :
	...
	for i in range(len(data.index)) :
		admis.append(data.loc[regionsSN[i]][0] * 100 / data.loc[regionsSN[i]][1])
	...
	mapSN.set_title('% de réussite au bac au Sénégal pour chaque région ('+str(year)+')')
	...
	for idx, row in regionsName.iterrows() :
		...
		mapSN.annotate(row['name'], xy=(x, y), xytext=(x, y), color='#ecf0f1', family='serif', fontweight='bold', horizontalalignment='center')

	## Sauvegarde dans les fichiers.png respectifs
	fileName = 'BAC_results_per_region_'+str(year)+'.png'
...
imageio.mimsave('BAC_results_per_region.gif', images, **kargs)